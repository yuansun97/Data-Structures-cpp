...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    // Push TreeNodes into the queue with increasing order of frequencies
    for (Frequency const & currFreq : frequencies) {
        TreeNode * currTreeNode = new TreeNode(currFreq);
        singleQueue.push(currTreeNode);
    }
    ...
    while (singleQueue.size() + mergeQueue.size() != 1) {
        ...
        subRoot = new TreeNode(leftNode->freq.getFrequency() + rightNode->freq.getFrequency());
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (currBit == 0) {
        ...
        currNode->left = readTree(bfile);
        currNode->right = readTree(bfile);
    } else {
        char currChar = bfile.getNextByte();
        currNode = new TreeNode(Frequency(currChar, 0));
    }
    return currNode;
}
