...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  for (unsigned i = 0; i < traversals_.size(); i++) {
    ...
    unsigned frame = 0;
    ImageTraversal * trav = traversals_[i];
    ColorPicker * cp = colorPickers_[i];

    for (const Point & point : (*trav)) {
      if (frame % frameInterval == 0) {
        animation.addFrame(png);
      }

      HSLAPixel & pix = png.getPixel(point.x, point.y);
      pix = cp->getColor(point.x, point.y);
      frame++;
    }
