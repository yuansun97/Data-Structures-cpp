...
class KDTree
{
  ...
  private:
     ...
     */
    // Swap the first point and second point in the vector.
    void swap(vector<Point<Dim>> & points, unsigned first, unsigned second);

    /** Partition the vector into two parts,
     *  so that points on the left are less than the pivot
     *  and points on the right are greater than the pivot under the given dimension d.
     * @param vector of points to partition, start and end index (inclusive), pivot index.
     * @return The index of the pivot after partition.
     * */
    unsigned partition(vector<Point<Dim>> & newPoints, unsigned start, unsigned end, unsigned pivotIdx, unsigned d);

    /** Find the k-th smallest point in the d-th dimension.
     * @param vector of points to select, start and end index (inclusive), k-th smallest, dimension d.
     * @return The k-th smallest point.
     * */
    Point<Dim> select(vector<Point<Dim>> & newPoints, unsigned start, unsigned end, unsigned k, unsigned d);

    /**
     * Recursive function to build K-d tree.
     *  @param vector ot points as treeNodes, current tree level to build, 
     *          subroot, start and end index in the newPoints
     * */
    void buildTree(vector<Point<Dim>> & newPoints, unsigned level, KDTreeNode *& subroot, unsigned start, unsigned end);

    void _copy(KDTree<Dim>::KDTreeNode *& currRoot, const KDTree<Dim>::KDTreeNode *& otherRoot);

    void _clear(KDTree<Dim>::KDTreeNode * subroot);

    /**
     * Search nearest point in K-d tree.
     * */
    void findNearest(KDTreeNode * subroot, unsigned level, Point<Dim> & currBest, const Point<Dim>& query) const;
};
