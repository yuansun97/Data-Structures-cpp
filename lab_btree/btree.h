...
size_t insertion_idx(const std::vector<T>& elements, size_t start, size_t end, const C& val) {
  ...
  if (elements[mid] > val) {
    if (mid == 0 || elements[mid - 1] < val) {
      return mid;
    } else {
      return insertion_idx(elements, start, mid - 1, val);
    }
  } else {
    if (mid == elements.size() - 1 || elements[mid + 1] > val) {
      return mid + 1;
    } else {
      return insertion_idx(elements, mid + 1, end, val);
    }
  }
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return insertion_idx(elements, 0, elements.size() - 1, val);
}
