...
{
    /* Your code goes here! */
    string upper_w1 = word1;
    string upper_w2 = word2;
    std::transform(upper_w1.begin(), upper_w1.end(), upper_w1.begin(), ::toupper);
    std::transform(upper_w2.begin(), upper_w2.end(), upper_w2.begin(), ::toupper);

    auto it_1 = dict.find(upper_w1);
    auto it_2 = dict.find(upper_w2);
    if (it_1 == dict.end() || it_2 == dict.end()) return false;
    if (it_1->second.size() == it_2->second.size()) {

        // std::cout << std::endl;
        // std::cout << "size = " << it_1->second.size() << std::endl;
        // std::cout << "words are : " << upper_w1 << " -- " << upper_w2 << std::endl;

        for (unsigned i = 0; i < it_1->second.size(); i++) {

            // std::cout << "i = " << i << ": " << it_1->second[i] << " -- " << it_2->second[i] << std::endl;

            if (it_1->second[i].compare(it_2->second[i]) != 0) {
