...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      distance = sqrt(((int)x - centerX) * ((int)x  - centerX) + ((int)y - centerY) * ((int)y - centerY));
      decreasePercentage = distance * 0.005;

      if (decreasePercentage < 0.8) {
        pixel.l = pixel.l * (1 -  decreasePercentage);
      } else {
        pixel.l = pixel.l * 0.2;
      }
    }
...
PNG illinify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    ...
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double distanceToBlue = std::min(360 - abs(pixel.h - 216), abs(pixel.h - 216));
      double distanceToOrange = std::min(360 - abs(pixel.h - 11), abs(pixel.h - 11));
