...
void heap<T, Compare>::heapifyDown(size_t currentIdx)
{
    ...
    if (!hasAChild(currentIdx)) 
        return;
    size_t priChildIdx = maxPriorityChild(currentIdx);
    if (higherPriority(_elems[priChildIdx], _elems[currentIdx])) {
        std::swap(_elems[priChildIdx], _elems[currentIdx]);
        heapifyDown(priChildIdx);
    }
}
...
heap<T, Compare>::heap(const std::vector<T>& elems) 
{
    ...
    for (auto t : elems) 
        _elems.push_back(t);
    size_t startHeapifyIdx = (_elems.size() - 1) / 2;
    for (int i = startHeapifyIdx; i >= 1; i--) {
        heapifyDown(i);
    }
}
