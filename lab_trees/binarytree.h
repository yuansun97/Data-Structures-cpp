...
class BinaryTree
{
    ...
    private:
        ...
        BinaryTree<T>::Node * mostRight(BinaryTree<T>::Node * subRoot) const;

        // Helper function 2 mostLeft, return the mostLeft node in the right subtree
        BinaryTree<T>::Node * mostLeft(BinaryTree<T>::Node * subRoot) const;

        // Helper function 3
        bool isOrderedRecursive(const BinaryTree<T>::Node * subRoot) const;

        // Helper function getPath
        void getPaths(std::vector<std::vector<T>>& paths, const BinaryTree<T>::Node * subRoot, int h) const;


        int sumDistances(BinaryTree<T>::Node * subRoot, int sum, int h) const;
