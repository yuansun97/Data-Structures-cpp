...
class InorderTraversal : public TreeTraversal<T> {
  public: 
    ...
    InorderTraversal(typename BinaryTree<T>::Node* root)
	    : root(root)
    {
      // Move to the most left child in the tree,
      // meanwhile store the subroots through the path in stack
      while (root != NULL) {
        stack.push(root);
        root = root->left;
      }
     ...
     */	
    // Used in Iterator & operator++()
    void add(typename BinaryTree<T>::Node *& treeNode) {
      // your code here
      typename BinaryTree<T>::Node * rightSubRoot = treeNode->right;
      while (rightSubRoot != NULL) {
        stack.push(rightSubRoot);
